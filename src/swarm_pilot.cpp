...
void SwarmFormationControl::on_relative_position(const swarm_msgs::swarm_fused & swarm_fused) {
...
void SwarmFormationControl::set_swarm_formation_mode(uint8_t _formation_mode, int master_id, int sub_mode, Eigen::Vector3d dpos, double dyaw) {
    ...
    if (formation_mode == drone_onboard_command::CTRL_FORMATION_HOLD_0 ||
        formation_mode == drone_onboard_command::CTRL_FORMATION_HOLD_1
    ) {
...
SwarmPilot::SwarmPilot(ros::NodeHandle & _nh):
    ...
    swarm_local_sub = nh.subscribe("/swarm_drones/swarm_drone_fused", 1, &SwarmFormationControl::on_relative_position, formation_control, ros::TransportHints().tcpNoDelay());
